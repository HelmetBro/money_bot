...
# DAVID: Take a look at data.py. the get function need to access that data object. that object is
# the algo.buy_and_sell_david_custom(self.ticker, start, end, self.data)  <-- self.data passed in here
# So that thing, and also the buy command. it's totally possible.
...
class DavidStrat(bt.Strategy):
    def next(self):
        ...
        start = end - datetime.timedelta(hours=1)
        print(self.data)
        print(self.data.close) # this line is stopping it idk lol
...
def run(ticker):
    ...
    data = DataFactory(dataname='AAPL',fromdate=START_DATE,todate=END_DATE, timeframe=bt.TimeFrame.Days)

    # adding strat + data
    cerebro = bt.Cerebro()
    cerebro.broker.setcash(10000.0)
    cerebro.addstrategy(SmaCross) #DavidStrat(ticker)
